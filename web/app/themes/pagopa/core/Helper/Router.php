...
final class Router
{
  ...
  public static function locateTemplate($default)
  {
    ...
    if (is_single()) {
      return TemplateLocator::get('single', 'pagopa', $queried_object, $default);
    }

    if (is_page()) {
      return TemplateLocator::get('page', 'pagopa', get_page_template(), $default);
    }

    if (is_category()) {
      return TemplateLocator::get('category', 'pagopa', $queried_object, $default);
    }

    if (is_tag()) {
      return TemplateLocator::get('tag', 'pagopa', $queried_object, $default);
    }

    if (is_tax()) {
      return TemplateLocator::get('taxonomy', 'pagopa', $queried_object, $default);
    }

    if (is_author()) {
      return TemplateLocator::get('author', 'pagopa', null, $default);
    }

    if (is_archive()) {
      return TemplateLocator::get('archive', 'pagopa', $queried_object, $default);
    }

    if (is_search()) {
      return TemplateLocator::get('search', 'pagopa', null, $default);
    }

    if (is_404()) {
      return TemplateLocator::get('404', 'pagopa', null, $default);
    }

    return  TemplateLocator::get('index', 'pagopa', null, $default);
  }
